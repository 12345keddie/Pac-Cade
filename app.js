...
async function startEmulator(core, file, theme) {
  ...
  if (String(core||'').toLowerCase() === 'segacd' || String(core||'').toLowerCase() === 'segacd') {
    window.EJS_biosUrl = encodeURI('bios_CD_E.bin');
    ...
    try {
      ...
      if (window.ROMDATA && window.ROMNAME) {
        ...
        style.textContent = `
          ...
          .gui_how_mobile, .gui_safari, .gui_title, .gui_controls, .gui_pleasewait { display: none !important; pointer-events: none !important; }
...
function setupNavigation() {
  ...
  const GamepadBridge = (() => {
    ...
    function isSF() { const b = base(); return ['ssf2t','sfiii3','mvsc'].includes(b); }
  ...
  function renderControllerMapping() {
    ...
    const defaults = { Up: 'KeyW', Down: 'KeyS', Left: 'KeyA', Right: 'KeyD', A: 'KeyX', B: 'KeyZ', Start: 'KeyS', Select: 'KeyA' };
  ...
  function gameActionTemplate() {
    ...
    if (core === 'mame32' || core === 'mame2003') {
      ...
      if (base === 'simpsons2p') {
        return [
          ...
          { name: 'Jump', target: 'KeyA', default: 'KeyK' },
          { name: 'Start', target: 'Enter', default: 'Enter' },
          { name: 'Coin', target: 'Digit1', default: 'Digit1' },
          { name: 'Service', target: 'F2', default: 'KeyP' },
        ];
    ...
    return [
      ...
      { name: 'Left', target: 'ArrowLeft', default: 'KeyA' },
      { name: 'Right', target: 'ArrowRight', default: 'KeyD' },
      { name: 'A', target: 'KeyX', default: 'KeyX' },
